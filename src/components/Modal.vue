        ...
        <div class="modal-body">
          <form @submit.stop.prevent="onSubmit">
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                <input
                  @keypress="isSpaceNotAllowed($event)"
                  id="plateNumber"
                  name="plateNumber"
                  v-model="form.plateNumber"
                  v-validate="{ required: true }"
                  :state="validateState('plateNumber')"
                  data-vv-as="Plate Number"
                  type="text"
                  class="form-control"
                  maxlength="20"
                  required
...
export default {
  ...
  methods: {
    isSpaceNotAllowed: function (event) {
      if(event.keyCode == 32) {
    ...
    park() {
      ...
      this.$parent.vehiclesParked.map(obj => {
        if (obj.plateNumber === this.form.plateNumber) {
          if(((new Date() - obj.timeOut)/1000) <= 3600) {
            this.returning = true
    ...
    unPark() {
      ...
      if (data) {
        const diff = new Date() - data.timeIn
        ...
        let park = this.$parent.park[a]
          for (let i = 0; i < park.length; i++) {
            let slot = park[i]
            slot.id === data.parkId ? this.$parent.park[a][i].occupied = false : ''
          }
        }
        this.resetForm()
        document.getElementById("close").click()
